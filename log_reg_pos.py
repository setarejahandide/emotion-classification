...
# Analyze the distribution of POS tags across emotion classes
def analyze_pos_distribution(pos_tags, labels):
    pos_dist = defaultdict(lambda: defaultdict(int))
    for tags, label in zip(pos_tags, labels):
        for word, tag in tags:
            pos_dist[label][tag] += 1
    return pos_dist

# Plot the POS distribution for each class
def plot_pos_distribution(pos_dist):
    pos_df = pd.DataFrame(pos_dist).fillna(0)
    pos_df = pos_df.div(pos_df.sum(axis=1), axis=0)  # Normalize counts
    pos_df.T.plot(kind='bar', stacked=True, figsize=(15, 7), colormap='tab20')
    plt.title('POS Tag Distribution by Emotion Class')
...
pos_dist_train = analyze_pos_distribution(train_pos_tags, train_labels)

# Plot POS distribution for training data
plot_pos_distribution(pos_dist_train)
